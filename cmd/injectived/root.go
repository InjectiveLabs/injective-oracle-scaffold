...
import (
	...
	"github.com/InjectiveLabs/injective-oracle-scaffold/version"
	...
	injectiveclient "github.com/InjectiveLabs/injective-oracle-scaffold/injective-chain/client"
)
...
func initRootCmd(rootCmd *cobra.Command, encodingConfig params.EncodingConfig) {
	...
	rootCmd.AddCommand(
		...
		injectiveclient.TestnetCmd(app.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
	)
...
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer, appOpts servertypes.AppOptions) servertypes.Application {
	...
	injectiveApp := app.NewInjectiveApp(
...
func createAppAndExport(
	...
	if height != -1 {
		injectiveApp = app.NewInjectiveApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), encCfg, appOpts)

		if err := injectiveApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		injectiveApp = app.NewInjectiveApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), encCfg, appOpts)
	}

	return injectiveApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
