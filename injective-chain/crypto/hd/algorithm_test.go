...
import (
	...
	_ "github.com/InjectiveLabs/injective-oracle-scaffold/injective-chain/crypto/codec"
...
func TestKeyring(t *testing.T) {
	...
	kr, err := keyring.New("injective", keyring.BackendTest, dir, mockIn, EthSecp256k1Option())
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, sidechain.BIP44HDPath, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	...
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, sidechain.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := hdwallet.MustParseDerivationPath(sidechain.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
